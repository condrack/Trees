...
//constructor initializes root
        tree(){root=NULL;counter=0;};
        void startnode(int x){ insertnode(root, x);};
        void printnode(){printtree(root);};
        void deletebegin(int x){deletenode(root,x);counter=rand()%100+1;};
...
//stores value returned by treemin() and treemax()
        int largest;
        int smallest;
...
//statement runs if node has two children
        ...
        {
//to alternate between the two delete methods
//a counter is randomly generated each time a node is deleted
//i check if the mod value is 0 or 1
            if(counter%2==0)
            {
                largest = treemax(match->left);
                deletenode(match,largest);
                match->data = largest;
            }
            else if(counter%2==1)
            {
                smallest = treemin(match->right);
                deletenode(match,smallest);
                match->data = smallest;
            }
...
int main()
{
    ...
    cout<<"*******************PART C*************************"<<endl;
    cout<<"The original tree in ascending order-"<<endl<<endl;
    branch.printnode();
    cout<<endl<<endl;
    cout<<"Deleting node with value 12. Resulting tree -"<<endl<<endl;
    branch.deletebegin(12);
    branch.printnode();
    cout<<endl<<endl;
    cout<<"Deleting node with value 30. Resulting tree -"<<endl<<endl;
    branch.deletebegin(30);
    branch.printnode();
    cout<<endl<<endl;
    cout<<"Deleting node with value 25. Resulting tree -"<<endl<<endl;
